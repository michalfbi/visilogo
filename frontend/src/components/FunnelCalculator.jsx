...
const FunnelCalculator = () => {
  ...
  const calculateFunnel = (e) => {
    ...
    setTimeout(() => {
      ...
      // Estymacja dla B2B (średni koszt pozyskania leada = 150 zł)
    ...
    <div className="min-h-screen bg-black pt-24 lg:pt-32 pb-12 lg:pb-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10 max-w-6xl">
        <div className="text-center mb-16">
          ...
          >
            <Calculator size={16} /> Narzędzie Analityczne B2B
          </motion.div>
          <motion.h1 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Kalkulator Budżetu <br/><span className="text-emerald-500">Marketingowego</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        <div className="grid lg:grid-cols-12 gap-12">
          ...
          >
            <form onSubmit={calculateFunnel} className="bg-[#0A0A0A] border border-white/10 p-8 rounded-2xl shadow-xl flex flex-col justify-center h-full relative overflow-hidden">
              <div className="mb-6">
                <label className="flex items-center gap-2 text-sm font-bold text-gray-400 uppercase tracking-wider mb-3"><Target size={16} className="text-emerald-500"/> Cel: Dodatkowy Przychód (Miesięcznie)</label>
              ...
              <div className="mb-6">
                <label className="flex items-center gap-2 text-sm font-bold text-gray-400 uppercase tracking-wider mb-3"><TrendingUp size={16} className="text-emerald-500"/> Średnia Wartość 1 Klienta (Koszyk)</label>
                <div className="relative">
              ...
              <div className="mb-8">
                <label className="flex items-center gap-2 text-sm font-bold text-gray-400 uppercase tracking-wider mb-3"><Users size={16} className="text-emerald-500"/> Skuteczność Zamknięć (Win Rate)</label>
                <div className="relative">
              ...
              <div className="mb-8 border-t border-white/10 pt-6">
                ...
                <input type="email" name="email" placeholder="Twój e-mail" value={formData.email} onChange={handleChange} required className="w-full bg-black border border-white/20 rounded-lg p-4 text-white focus:outline-none focus:border-emerald-500 transition-colors" />
              </div>
              ...
              <button type="submit" disabled={status === 'calculating'} className={`w-full flex items-center justify-center gap-3 font-bold py-5 px-8 rounded-lg transition-all text-lg ${status === 'calculating' ? 'bg-gray-800 text-gray-500' : 'bg-emerald-600 text-white hover:bg-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.3)]'} mt-auto`}>
                {status === 'calculating' ? <><Activity size={20} className="animate-pulse" /> Przeliczanie...</> : <><Calculator size={20} /> Oblicz Mój Budżet</>}
              </button>
            </form>
          </motion.div>
            ...
            <AnimatePresence mode="wait">
              {status === 'idle' && (
                ...
                >
                  ...
                  <p className="max-w-md">Uzupełnij parametry swojego biznesu po lewej stronie, aby system przeprowadził inżynierię wsteczną Twojego lejka sprzedażowego.</p>
                </motion.div>
              )}
              ...
              {status === 'complete' && results && (
                ...
                >
                  ...
                  <div className="bg-emerald-500/5 border border-emerald-500/30 p-8 rounded-xl relative shadow-2xl">
                    <div className="text-emerald-500 text-xs font-bold uppercase tracking-wider mb-6 flex items-center gap-2"><CheckCircle size={14}/> Twój Raport Gotowy</div>
                    
                    <div className="grid sm:grid-cols-2 gap-8 mb-8">
                      <div>
                        <div className="text-gray-400 text-sm mb-1">Potrzebujesz nowych klientów</div>
                        <div className="text-3xl font-black text-white">{results.clientsNeeded} <span className="text-sm font-normal text-gray-500">osób/mc</span></div>
                      </div>
                      <div>
                        <div className="text-gray-400 text-sm mb-1">Potrzebujesz zapytań (Leads)</div>
                        <div className="text-3xl font-black text-emerald-400">{results.leadsNeeded} <span className="text-sm font-normal text-gray-500">leadów/mc</span></div>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-white/10 pt-6">
                      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                        <div>
                          <div className="text-gray-400 text-sm mb-1">Rekomendowany Budżet na Reklamy</div>
