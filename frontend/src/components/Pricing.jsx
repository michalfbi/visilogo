...
import { CheckCircle, ChevronDown, ChevronUp, ArrowRight, AlertTriangle, Truck, Palette, Zap, ShieldCheck } from 'lucide-react';
import { Link } from 'react-router-dom';
import { pricingPlans, localPricingPlans, printPricingPlans, singleServices } from '../mock';
...
const PricingCard = ({ plan, index }) => {
  ...
  return (
    ...
    >
          ...
          <AnimatePresence>
            {isExpanded && (
              <motion.div
                ...
                transition={{ duration: 0.3 }} className="space-y-4 pt-4 overflow-hidden"
              >
      ...
      >
        {isExpanded ? <>Zwiń listę <ChevronUp size={16} /></> : <>Zobacz pełną listę (+{plan.features.length - 4}) <ChevronDown size={16} /></>}
      </button>
      ...
      <Link 
        ...
        className={`mt-auto block text-center w-full py-4 text-sm font-bold uppercase tracking-widest transition-all ${
          plan.highlight ? 'bg-[#00FFD1] text-black hover:bg-[#00FFD1]/90' : 'border border-white/20 text-white hover:border-[#00FFD1] hover:text-[#00FFD1]'
        }`}
      >
        Wybierz ten pakiet
      </Link>
        ...
        <div className="border-t border-white/10 pt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-[1400px] mx-auto">
            {singleServices.map((category, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="bg-[#050505] border border-white/5 p-8 rounded-xl">
                <h4 className="text-[#00FFD1] font-bold uppercase tracking-widest text-[11px] mb-6 border-b border-white/10 pb-4">{category.category}</h4>
                <ul className="space-y-6">
                  {category.items.map((item, itemIdx) => (
                    <li key={itemIdx} className="group">
                      <div className="flex flex-col mb-1 gap-1"><span className="text-white font-bold text-sm">{item.name}</span><span className="text-gray-500 font-mono text-xs">{item.price}</span></div>
                      <p className="text-gray-500 text-xs leading-relaxed group-hover:text-gray-400 transition-colors mt-2">{item.desc}</p>
                    </li>
                  ))}
                </ul>
              </motion.div>
