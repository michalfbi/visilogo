...
import { ShieldAlert, MailX, ArrowRight, CheckCircle, Lock, Loader2, Activity, ShieldCheck, Mail } from 'lucide-react';

const WEBHOOK_URL = "https://hook.eu1.make.com/we5gnbk29ew8kcg4s64vi1xon7ig4pjs";

// Baza polskich "Spam Words"
const SPAM_WORDS = [
...
const SpamScanner = () => {
  ...
  const [status, setStatus] = useState('idle'); // idle, scanning, locked_results, complete
  ...
  const analyzeEmail = (text) => {
    ...
    SPAM_WORDS.forEach(word => {
      ...
      if (matches) {
        ...
        if (!foundIssues.includes("Użyto agresywnych słów sprzedażowych (tzw. Spam Words)")) {
          foundIssues.push("Użyto agresywnych słów sprzedażowych (tzw. Spam Words)");
        }
        // Przygotowanie tekstu z podświetleniem
    ...
    // 2. Szukanie nadużywania CAPS LOCKA
    ...
    // 3. Szukanie znaków zapytania / wykrzykników pod rząd
    if (/(!!+|!\\?|\\?\\?+)/.test(text)) {
      score -= 10;
  ...
  const handleUnlock = (e) => {
    ...
    fetch(WEBHOOK_URL, {
      ...
      body: JSON.stringify({
        form_type: "Lead z Narzędzia: Skaner Anty-Spamowy",
        email_klienta: userEmail.trim(),
        wynik_skanera: analysis.score,
        message: `Mocny Lead! Klient (${userEmail}) sprawdza swoje maile pod kątem SPAMu. Zdobądź go, oferując mu pomoc w optymalizacji kampanii lub budowie strony.`
      })
    ...
    <div className="min-h-screen bg-black pt-32 pb-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10 max-w-6xl">
        <div className="text-center mb-16">
          ...
          >
            Twoje maile B2B trafiają w próżnię? Algorytmy Google i Outlooka są bezlitosne. Wklej treść swojej wiadomości, a my w 3 sekundy wskażemy słowa, przez które wpadasz do SPAMu.
          </motion.p>
        </div>

        <div className="grid lg:grid-cols-12 gap-12">
          {/* Formularz - Wklejanie maila */}
          <motion.div 
            ...
            <div className="bg-[#0A0A0A] border border-white/10 p-8 rounded-2xl shadow-xl flex flex-col h-full relative overflow-hidden">
              <form onSubmit={handleScan} className="flex flex-col h-full">
                <div className="mb-6 flex-grow flex flex-col">
                  ...
                  <textarea 
                    placeholder="Wklej tutaj treść wiadomości, którą wysyłasz do klientów..." 
                    value={emailText}
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-3">Nasze filtry szukają zakazanych polskich słów sprzedażowych, błędów interpunkcyjnych i manipulacji formatowaniem.</p>
                </div>
          ...
          >
            <AnimatePresence mode="wait">
              {status === 'idle' && (
                ...
                >
                  Wklej tekst wiadomości i kliknij "Przeskanuj", aby wykryć zagrożenia filtrami antyspamowymi.
                </motion.div>
              )}
              ...
              {status === 'scanning' && (
                ...
                >
                  ...
                  <p className="text-red-400 text-sm font-mono h-6 mb-8">Wykrywanie sygnatur SPAMu...</p>
                ...
                >
                  <div className="bg-white/5 border border-red-500/30 p-8 rounded-xl relative shadow-2xl flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-white mb-8">
                      Wykryto <strong className="text-red-500">{analysis.issuesCount} zagrożeń</strong>, przez które Twój mail może wpaść do folderu SPAM lub Oferty.
                    </p>
                    ...
                    <form onSubmit={handleUnlock} className="w-full max-w-sm bg-black p-6 rounded-xl border border-white/10 relative overflow-hidden">
                      ...
                      <h4 className="text-sm font-bold text-white mb-4">Odblokuj szczegółowy raport</h4>
                      <input 
                        type="email" 
                ...
                >
                  <div className="bg-[#0A0A0A] border border-white/10 p-6 rounded-xl relative shadow-xl flex-grow flex flex-col">
                    <div className="flex justify-between items-center mb-4 border-b border-gray-800 pb-4">
                      <div className="text-gray-400 text-xs font-bold uppercase tracking-wider flex items-center gap-2">Raport Analizy Tekstu</div>
                    ...
                    <div className="bg-white p-4 rounded-lg text-black text-[13px] leading-relaxed font-sans overflow-y-auto max-h-[250px] border border-gray-200" dangerouslySetInnerHTML={{ __html: analysis.highlightedText }}>
                    </div>
                    <div className="mt-3 text-xs text-gray-500 flex gap-4">
                      <span className="flex items-center gap-1"><span className="w-2 h-2 bg-red-500 rounded-full"></span> Słowa spamowe</span>
                      <span className="flex items-center gap-1"><span className="w-2 h-2 bg-yellow-500 rounded-full"></span> CAPS LOCK</span>
                      <span className="flex items-center gap-1"><span className="w-2 h-2 bg-orange-500 rounded-full"></span> Interpunkcja</span>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-400 mb-4 text-xs leading-relaxed">
                          Wyłapałeś błędy i Twój mail ominął SPAM. Super. Ale kiedy prezes w niego kliknie i zobaczy starą, wolno ładującą się stronę WWW... natychmiast ją zamknie. <strong className="text-white">Nie przepalaj trudu włożonego w wiadomości słabą witryną.</strong>
                        </p>
