...
const Layout = ({ children }) => {
  ...
  const navLinks = [
    ...
    { name: '🔥 POJEDYNEK', href: '/pojedynek', highlight: true },
  ];
          ...
          <div className="hidden lg:flex items-center gap-12">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href} 
                className={`text-xs font-bold tracking-[0.2em] transition-colors ${link.highlight ? 'text-[#00FFD1] hover:text-white' : 'text-gray-400 hover:text-white'}`}
              >
                {link.name}
              </a>
            ))}
