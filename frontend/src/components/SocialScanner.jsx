...
const SocialScanner = () => {
  ...
  const handleScan = async (e) => {
    e.preventDefault();
    if (!url) {
      setError('Wpisz adres strony, aby rozpocz膮膰 skanowanie.');
      return;
    }
    ...
    setError('');
    setPreviewData(null);
    setImageFailed(false); // Resetujemy b艂膮d obrazka przy nowym skanowaniu
    ...
    try {
      ...
      if (data.status === 'success') {
        setPreviewData({
          title: data.data.title || 'Brak tytu艂u strony',
          description: data.data.description || 'Brak opisu (Description tag). Tw贸j link wygl膮da podejrzanie.',
          image: data.data.image?.url || null,
          url: data.data.url,
          publisher: data.data.publisher || new URL(targetUrl).hostname
        });
      } else {
        throw new Error("Nie uda艂o si臋 pobra膰 danych strony. Prawdopodobnie serwer blokuje dost臋p lub domena nie istnieje.");
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-black pt-24 lg:pt-32 pb-12 lg:pb-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10 max-w-4xl">
        <div className="text-center mb-16">
          ...
          >
            <Eye size={16} /> Skaner Pierwszego Wra偶enia
          </motion.div>
          <motion.h1 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Sprawd藕, czy Tw贸j link <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-500">odstrasza inwestor贸w</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          >
            {loading ? <><Loader2 className="animate-spin" size={24} /> Generowanie podgl膮du...</> : <><Smartphone size={24} /> Wygeneruj Podgl膮d B2B</>}
          </button>
        </motion.form>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="bg-[#1D2226] rounded-xl overflow-hidden shadow-2xl border border-gray-700">
                  <div className="p-4 border-b border-gray-700 flex items-center gap-2">
                    ...
                    <span className="text-white font-bold text-sm">Wiadomo艣膰 LinkedIn</span>
                  </div>
                    ...
                    <div className="w-full h-40 bg-gray-100 -mt-4 -mx-4 mb-4 border-b border-gray-200 relative flex items-center justify-center overflow-hidden">
                      {hasValidImage ? (
                        ...
                        <div className="flex flex-col items-center text-red-500 p-4 text-center">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest">Brak lub zepsute zdj臋cie</span>
                        </div>
                      )}
                    </div>
              ...
              <div className={`p-8 rounded-2xl border flex flex-col md:flex-row items-center justify-between gap-8 ${hasValidImage ? 'bg-[#00FFD1]/5 border-[#00FFD1]/20' : 'bg-red-500/5 border-red-500/30 shadow-[0_0_30px_rgba(239,68,68,0.15)]'}`}>
                <div>
                  <h4 className="text-2xl font-bold text-white mb-2 flex items-center gap-2">
                    {hasValidImage ? <><CheckCircle className="text-[#00FFD1]" /> Tagi dzia艂aj膮 poprawnie</> : <><AlertTriangle className="text-red-500" /> Tw贸j link wygl膮da jak SPAM!</>}
                  </h4>
